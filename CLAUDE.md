...
#### 双源持仓计算系统
- **交易记录计算**: 使用加权平均成本法基于历史交易记录计算持仓
- **API实时余额**: 通过币安API获取账户实际持仓余额
- **智能整合**: 优先使用API实际余额，同时显示与交易记录计算的差异
- **透明展示**: 区分"实际持仓"(API余额)与"交易计算持仓"(基于交易记录)
- **差异标识**: 当存在持仓差异时，标注可能的其他来源(充值、提现、手续费等)
- **价值计算**: 持仓价值基于API实际余额 × 当前市价，确保准确性
- 核心逻辑在`common/utilities.py:format_currency_report()`和`format_trades_details()`中实现
